...
def create_app(settings=None, rate_limit: int = RATE_LIMIT,
    ...
    origins = [o for o in os.getenv("SIGNALGATE_CORS_ORIGINS",
    ...
    def _find_results() -> Path | None:
        """Eval outputs land under artifacts/<dir>/agent/results.jsonl depending
        on which make target produced them; accept the known layouts."""
        for cand in ("agent", "eval", "holdout", "ci"):
            p = settings.artifacts_dir / cand / "agent" / "results.jsonl"
            if p.exists():
                return p
        return None
    ...
    def api_metrics() -> JSONResponse:
        """Published evaluation numbers (regenerated by make eval)."""
        out: dict = {}
        for name in ("metrics", "metrics_holdout", "ablation_metrics"):
            p = _reports_dir() / f"{name}.json"
            if p.exists():
                out[name] = json.loads(p.read_text(encoding="utf-8"))
        ds = _reports_dir() / "dataset.md"
        if ds.exists():
            out["dataset_report"] = ds.read_text(encoding="utf-8")
    ...
    def api_digest() -> JSONResponse:
        ...
        p = _find_results()
        records = load_records(p) if p else []
        return JSONResponse({
            "n": len(records),
            "rejected": sum(1 for r in records if r["verdict"] == "REJECT_SPURIOUS"),
            "review": sum(1 for r in records if r["verdict"] == "NEEDS_REVIEW"),
            "promising": sum(1 for r in records if r["verdict"] == "PROMISING"),
            "records": records,
        })

    @app.get("/api/runs")
    def api_runs(limit: int = 12) -> JSONResponse:
            ...
            for d in sorted(runs.iterdir(), key=lambda p: p.name)[-limit:]:
                ...
                if f.exists():
                    try:
                        ...
                        out.append({
                            "run_id": b["run_id"], "name": b["spec"]["name"],
                            "verdict": b["verdict"],
